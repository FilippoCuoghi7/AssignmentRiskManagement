function P = compute_KL_price_EquityTranche(I, rec, Ku, rho, pd, dates, discounts, ...
    refDate, interpDate)
% COMPUTE_EXACT_PRICE Calculates the present value (price) of a CDO equity tranche
...
%   Ku         - Detachment point (upper bound) of the CDO equity tranche.
...
%                the equity tranche, expressed as a percentage of the face value.

% Create an anonymous function locking in the portfolio parameters
integrand_KL = @(z) compute_KL_integrand(z, I, rec, Ku, 0, rho, pd);

% Calculate the unconditional expected tranche loss (as a percentage)
KL_expected_tranche_loss = quadgk(integrand_KL, 1/(2*I), 1);
