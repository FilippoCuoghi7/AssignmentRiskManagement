function P = compute_exact_price(I, rec, Ku, Kd, rho, pd, dates, discounts, refDate, interpDate)
% COMPUTE_EXACT_PRICE Calculates the present value (price) of a CDO tranche
% as a percentage of its face value using the Exact Gaussian Copula model.
%
% INPUTS:
...
%   discounts  - An array of discount factor corresponding to the yield curve 'dates'.
...
integrand_exact = @(y) compute_exact_integrand(y, I, rec, Ku, Kd, rho, pd);

% Calculate the unconditional expected tranche loss (as a percentage)
exact_expected_tranche_loss = quadgk(integrand_exact, -Inf, Inf);
...
P = df * (1 - exact_expected_tranche_loss);
